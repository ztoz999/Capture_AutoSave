...
def Autocap():
    ...
    start, init = round(time.time()), round(time.time())

    try:
        elapsed = int(cap_spintime.get()) * 60

        while True:
            time.sleep(0.01)
            cap_info.config(text="자동 저장중\n중지하려면 <F10> 키를 누르세요\n남은 시간 : {}초".format(elapsed - (start - init)))

            if keyboard.is_pressed("F10") == True or elapsed <= start - init:
                cap_info.config(text="")
                keyboard.remove_hotkey("F8")
                break
            elif round(time.time()) == start + autosave:
...
# def Keycancel():
#     """ 캡쳐룰 중지하고, 키보드 이벤트 을 제거 """

#     if cap_autosave.get() != "수동":
#         messagebox.showinfo("중자", "중지하려면 F10 키를 누르세요")
#     else:
#         return None
...
def mg_Merge():
    ...
    tkinter.Label(toplevel, image="::tk::icons::information",
                  bg="#345").pack(padx=5, pady=5)
    tkinter.Label(toplevel, text="병합중", foreground="white",
                  bg="#345").pack(fill="both", expand=True)
...
def mg_MergeImage(progressbar:ttk.Progressbar):
    ...
    try:
        ...
        # 병합할 이미지 켄버스 만들기
        mergedimag = Image.new(mode="RGB", size=(
            maxwidth, totalheight), color=(0, 0, 0))
...
def SelfileShow(event: tkinter.Event, tab: str):
    ...
    if selection != ():
        try:
            ...
            img_showinfo.config(text="SIZE = {} x {}".format(
...
cap_saveframe = tkinter.LabelFrame(
    captureframe, text="저장경로를 설정하세요", font=fontname)
...
selectbutton = tkinter.Button(
    cap_saveframe, text="폴더 선택", font=fontname, command=SelFolder)
...
var_autosave = ["수동", "5초", "10초", "30초", "60초", "180초"]
